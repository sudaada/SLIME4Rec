...
class AbstractTrainer(object):
    ...
    def __init__(self, config, model, vice_model):
        self.config = config
        self.model = model
        self.vice_model = vice_model
...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model, vice_model):
        super(Trainer, self).__init__(config, model, vice_model)
        ...
        model_dir = config['log_dir'] + '-' + str(config['n_layers']) + '-' + config['filter_mixer'] + '-' + str(config['global_ratio'])
        self.saved_model_file = os.path.join(model_dir, 'model.pth')
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        """
        self.model.train()
        # self.vice_model是不参与训练的，只是复制model的加噪参数然后生成y2
        ...
        for batch_idx, interaction in iter_data:  # 本质上还是遍历train_data

            interaction = interaction.to(self.device)
            self.optimizer.zero_grad()
            losses = loss_func(interaction, self.vice_model)
