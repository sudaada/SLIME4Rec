...
class SequentialDataLoader(AbstractDataLoader):
    ...
    def __init__(self, config, dataset, batch_size=1, dl_format=InputType.POINTWISE, shuffle=False, phase='train'):
        ...
        self.phase = phase
        if config['SSL_AUG'] == 'SLIME4Rec' and self.phase == 'train':
    ...
    def data_preprocess(self):
        ...
        if self.config['model'] == 'SLIME4Rec':
            # 数据集之间不一样
    ...
    def _shuffle(self):
        if self.real_time:
            ...
            if self.config['SSL_AUG'] == 'SLIME4Rec' and self.phase == 'train':
    ...
    def _get_processed_data(self, index):
        ...
        elif self.config['SSL_AUG'] == 'SLIME4Rec' and self.phase == 'train':
