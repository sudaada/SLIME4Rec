...
class AbstractRecommender(nn.Module):
    ...
    def calculate_loss(self, interaction, vice_model):
...
class SequentialRecommender(AbstractRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        # load dataset info
        self.USER_ID = config['USER_ID_FIELD']  # 指定用户id域 user_id
        self.ITEM_ID = config['ITEM_ID_FIELD']  # 指定物品id域 item_id
        self.ITEM_SEQ = self.ITEM_ID + config['LIST_SUFFIX']  # item_id_list
        self.ITEM_SEQ_LEN = config['ITEM_LIST_LENGTH_FIELD']  # 指定序列长度域 item_length
        self.POS_ITEM_ID = self.ITEM_ID  # 指定物品id域 item_id
        self.NEG_ITEM_ID = config['NEG_PREFIX'] + self.ITEM_ID  # 指定负采样前缀 neg_item_id
        self.max_seq_length = config['MAX_ITEM_LIST_LENGTH']  # 指定最大序列长度50
        self.n_items = dataset.num(self.ITEM_ID)  #
