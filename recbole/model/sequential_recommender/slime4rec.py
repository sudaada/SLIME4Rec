...
from recbole.model.layers import Encoder
...
class SLIME4Rec(SequentialRecommender):
    ...
    def gen_ran_output(self, item_seq, item_seq_len, vice_model):
        for (adv_name, adv_param), (name, param) in zip(vice_model.named_parameters(), self.named_parameters()):
            if name.split('.')[0] == 'proj_head':
                adv_param.data = param.data
            else:
                adv_param.data = param.data + self.simgrace_eta * torch.normal(0, torch.ones_like(param.data) * param.data.std()).to('cuda')
        seq_output_vice = vice_model.forward(item_seq, item_seq_len)
        return seq_output_vice
